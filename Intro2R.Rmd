...
The following provides very brief introduction to R using a tabular file as an example [@manualr]. 
...
used for this. The class of resulting object is a `data.frame`. Its dimensions can
be checked with the `dim` function, here 150 rows and 5 columns. To print only
...
The following performs some basic calculations on the imported object. First, `df`
is converted to a numeric `matrix`, and then the mean (sum) is calculated for the 
...
ma[1:4,]
mean(ma[1,])
rowMeans(ma)[1:10] # print first 10 of 150 values
rowSums(ma)[1:10] # print first 10 of 150 values
```
...
Plots can be saved to files in different graphics formats using R functions with the corresponding
names, including: `pdf`, `svg`, `tiff`, `png`, `jpg`, etc. The file name is specified under the
these function's arguments (here `myplot.png`).
...
For instance, `?t.test` will open the help file for Student's t-Test in R.

## Packages

Many R packages (libraries) can be installed with the `install.packages` or the `BiocManager::install` functions.
Before a package can be used, it needs to be loaded with the `library` function.
...
of the UCR GEN242 class. Additional tutorials are linked [here](https://pairach.com/2012/02/26/r-tutorials-from-universities-around-the-world/).
