...
The following will install all packages required for this exercise. The install in new 
R project will take about 2-3 minutes. This install only needs to be done once, meaning
...
rows). In this case the authors have normalized the already with the RMA
method [@Gautier2004-tk]. This is why the normalization is skipped here. Note, RMA returns log2 transformed values.
Users who are interested in learning to perform the normalization themselves want to look into 
the `rma` function that is part of the `affy` package from Bioconductor (see [here](https://bioconductor.org/packages/release/bioc/html/affy.html)).
...
package [@Ritchie2015-rl]. The includes the following steps: (1) creation of a
